...
**우리 1단계(θ가 이산값 몇 개로 주어짐) 조건에서는 RMCM이 CORDIC보다 더 잘 맞을 가능성**이 있음(반복이 필요 없어 지연이 짧음) — 실측 PPA로 ①·②·③을 비교하는 것이 다음 설계 단계의 실험 후보. **(2026-09-08 실측으로 확정: RMCM 압승, §110 참고.)**

### 2-C. 2단계(회전각 직접 추정) 세 후보 — 회로화 난이도 관점 비교, 1단계 완료 후 다음 설계용 사전정리

1단계(RMCM 채택, 통합검증 완료 — progress.md §105~111)를 마무리하면서, 2단계로 넘어가기 전에 2-2/2-3/2-4(D2_01/D2_02/D2_04) 세 논문을 "이걸 그대로 회로로 옮기면 뭐가 제일 힘든가" 기준으로 다시 훑음. 셋 다 순수 소프트웨어(부동소수점, CPU/GPU 알고리즘)라 회로화 난이도는 논문에 안 나옴 — 직접 판단 필요.

| | Kim2014 (파티클 필터) | CMax-SLAM (contrast max + B-spline BA) | EROAM (구면 ICP) |
|---|---|---|---|
| 핵심 연산 | 후보 회전(파티클) N개마다 이벤트 적합도 채점 + 가중 리샘플링, 지도는 칼만필터로 셀별 갱신 | 비선형 최적화(Gauss-Newton류)로 B-spline 제어점 전체 재조정 | 이벤트를 구면에 투영 -> k-d tree 최근접점 탐색 -> 반복 정합(ICP) |
| 반복/재귀 구조 | 매 이벤트마다 N개 파티클 갱신 + 주기적 리샘플링(정렬/CDF 필요) | 최적화 자체가 반복(iterative) — 수렴할 때까지 여러 번 | ICP 자체가 반복(수렴할 때까지) + 트리 갱신도 반복 |
| 회로에 가장 걸리는 부분 | 리샘플링(가중치 기반 후보 선택 — 정렬/누적분포, 조합논리로 만들기 번거로움) | 정규방정식 풀이(행렬 역산급) — 사이클당 스트리밍 처리와 근본적으로 안 맞음, 사실상 배치/오프라인 성격 | k-d tree(포인터 기반 자료구조) 자체가 고정 크기 회로로 옮기기 가장 까다로움 |
| 우리 상황에 유리한 점 | 파티클을 우리 theta_idx LUT(256개 이산값)로 그대로 대응 가능 — "파티클 리샘플링" 대신 "256개 후보 전부 채점 후 최댓값 선택"으로 바꾸면 정렬/CDF가 필요 없어짐(비교기 트리로 충분) | 없음(가장 무거움) | "최근접점 탐색"을 k-d tree 대신 **우리가 이미 갖고 있는 64x64 world_mem의 규칙적인 격자**로 바꾸면(이벤트를 world grid에 투영해서 그 칸에 이미 쓰여있는 값과 비교) k-d tree 없이 동일한 아이디어를 구현 가능 |

**잠정 결론(다음 2단계 설계 논의의 출발점, 아직 확정 아님)**: CMax-SLAM의 번들 조정은 우리 스트리밍 회로 구조와 근본적으로 안 맞아 보임(비선형 최적화는 학생 ASIC 규모에서 회로화 난이도가 지나치게 큼) — 후순위. Kim2014와 EROAM은 둘 다 핵심 아이디어를 **"이산 후보를 우리가 이미 만든 인프라(theta_idx LUT, world_mem 격자)에 그대로 얹는" 방식으로 순화**할 여지가 있어 보임: 매 윈도우(또는 매 사이클)마다 들어온 이벤트 배치를 **256개 후보 theta_idx 전부**(또는 일부 부분공간)로 coord_transform_rmcm에 통과시켜, 각 후보가 만든 world 좌표가 **이미 world_mem에 쓰여있는 값과 얼마나 잘 맞는지**(polarity 일치 개수 등 단순 채점) 점수를 매기고, 최고점 후보를 이번 사이클의 회전각으로 채택 — Kim2014의 "파티클 채점"과 EROAM의 "기존 지도와 대조"를 우리 인프라(1단계에서 이미 검증된 coord_transform_rmcm + world_mem_writer) 위에서 합친 형태. 이건 **아이디어 스케치일 뿐, 실제 설계 확정은 2단계 착수 시 사용자와 다시 기획 단계를 거쳐야 함**(1단계 때와 동일한 순서: 문헌조사 -> 설계기획 -> 승인 -> 구현).
