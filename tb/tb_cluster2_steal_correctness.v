// cluster2_steal 정확성 검증 -- tb_cluster2_correctness.v와 동일 방법론(event_scoreboard).
`timescale 1ns/1ps
module tb_cluster2_steal_correctness;
  parameter N = 16;
  ...
  aer_tx16_trad_rowcol_fovea_cluster2_steal dut(
  ...
  initial begin
    ...
    for (cyc = 0; cyc < CYCLES; cyc = cyc + 1) begin
      ...
      // 두 lane이 같은 사이클에 같은 행을 가리키면 안 됨(steal 로직 자체 모순 검증).
    ...
    if (error_count == 0)
      $display("CLUSTER2_STEAL_CORRECTNESS_PASS");
    else
      $display("CLUSTER2_STEAL_CORRECTNESS_FAIL errors=%0d", error_count);
    $finish;
  end
endmodule
