// coord_transform_cordic 실트래픽 검증 -- 실제 UZH shapes_rotation 4x4 patch 이벤트
...
`timescale 1ns/1ps
module tb_coord_transform_cordic_uzh_trace;
  reg clk = 0;
  reg rst = 1;
  ...
  initial begin
    ...
    fh = $fopen("common_traces_uzh/uzh_shapes_rotation_patch.coord_transform_cordic_vectors.txt", "r");
    if (fh == 0) begin
      $display("FAIL: cannot open coord_transform_cordic_vectors.txt");
      $finish;
    end
    ...
    if (total == 8503 && mismatches == 0)
      $display("COORD_TRANSFORM_CORDIC_UZH_TRACE_PASS");
    else
      $display("COORD_TRANSFORM_CORDIC_UZH_TRACE_FAIL");
    $finish;
  end
endmodule
