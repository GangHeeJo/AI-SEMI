// aer_tx16_adaptive_v2 표준 무작위 트래픽 스트레스 테스트 (phantom 체크 + 5개 지표).
module tb_bench_adaptive_v2;
  `ifndef SEED_VAL
  `define SEED_VAL 1
  `endif
  ...
  initial begin
    rst = 1; req = 16'd0; new_event = 16'd0; total_grants = 0; overflow_warns = 0; phantom_errors = 0;
    ...
    print_report;
    if (phantom_errors > 0)
      $display("=== 검증 실패: phantom 이벤트 %0d건 ===", phantom_errors);
    else
  ...
  task print_report;
    ...
    begin
      ...
      $display("=== ADAPTIVE_V2 BENCH REPORT (CYCLES=%0d, ARRIVAL_PCT=%0d%%) ===", CYCLES, ARRIVAL_PCT);
      ...
      $display("[5] 그룹간 평균 지연시간: center(2x2)=%0d cycles, periphery=%0d cycles",
