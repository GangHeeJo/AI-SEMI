// aer_tx16_adaptive_v2_lite 기능 정확성 검증 — v2 원본 correctness tb를 그대로 재사용,
// DUT만 lite(비트폭 축소판)로 교체. 알고리즘은 v2와 동일해야 하므로 PASS 조건도 동일.
module tb_aer16_adaptive_v2_lite_correctness;
  parameter QDEPTH = 64;
  reg clk = 0;
  reg rst;
  reg [15:0] req;
  ...
  `endif
  aer_tx16_adaptive_v2_lite #(.WEIGHT(`WEIGHT_VAL), .DECAY_SHIFT(`DECAY_SHIFT_VAL)) tx(.clk(clk), .rst(rst), .req(req), .new_event(new_event), .valid(valid), .addr_type(addr_type), .addr(addr));
