// cluster2_buf(steal 없는 버전) 정확성 검증 -- tb_steal_buf_correctness.v와 동일
// 방법론(arrival 펄스, shadow_cnt 2-deep 포화, overrun 엣지 전 샘플링), steal 관련
// 디버그 출력만 제거.
`timescale 1ns/1ps
module tb_cluster2_buf_correctness;
  parameter N = 16;
  ...
  aer_tx16_trad_rowcol_fovea_cluster2_buf dut(
  ...
  reg was_overrun [0:15]; // 이번 사이클에 overrun으로 거부됐는지(드레인 후 shadow_cnt
                           // 재확인 시 이중계산 방지 -- §버그: drain_lane이 shadow_cnt를
                           // 먼저 깎아버리면 "가득 찼었는지" 판정이 오염됨).
  ...
  initial begin
    ...
    for (cyc = 0; cyc < CYCLES; cyc = cyc + 1) begin
      ...
      // was_overrun[i](엣지 전 스냅샷)로 판정 -- drain_lane이 이미 shadow_cnt를 깎아놔서
      // "가득 찼었는지"를 지금 다시 물으면 오염됨(위 버그 참고).
      for (i = 0; i < 16; i = i + 1) begin
    ...
    if (error_count == 0) $display("CLUSTER2_BUF_CORRECTNESS_PASS");
    else $display("CLUSTER2_BUF_CORRECTNESS_FAIL errors=%0d", error_count);
    $finish;
  end
endmodule
