// coord_transform_rotate2d 전수 검사(4x4 x 256 theta = 4096가지 전부).
...
`timescale 1ns/1ps
module tb_coord_transform_rotate2d_correctness;
  reg clk = 0;
  reg rst = 1;
  reg valid_in = 0;
  ...
  initial begin
    ...
    if (total == 4096 && mismatches == 0)
      $display("COORD_TRANSFORM_EXHAUSTIVE_PASS");
    else
      $display("COORD_TRANSFORM_EXHAUSTIVE_FAIL");
    $finish;
  end
endmodule
