// repeat-flag 코덱을 공식 50-workload trace로 검증 + 실제 비트절감 측정.
// admission 순서는 §82에서 잡은 버그와 같은 원칙(pending 클리어를 새 도착 처리보다
// 먼저) 적용. 디코더에는 repeat 사이클에 row/col_mask를 지운 값을 넣어 진짜 복원인지
// 확인.
`timescale 1ns/1ps
module tb_repeat_encode_common_trace;
  ...
  initial begin
    generated = 0; overrun = 0; acked = 0; native_bits = 0; repeat_bits = 0;
    ...
    $display("TRACE=%0s generated=%0d overrun=%0d acked=%0d native_bits=%0d repeat_bits=%0d repeat_hits=%0d decode_mismatch=%0d",
      trace_file_r, generated, overrun, acked, native_bits, repeat_bits, repeat_hits, decode_mismatch);
    if (decode_mismatch == 0 && acked == generated - overrun)
      $display("REPEAT_TRACE_PASS %0s", trace_file_r);
    else
      $display("REPEAT_TRACE_FAIL %0s", trace_file_r);
    $fclose(fd);
    $finish;
  end
endmodule
