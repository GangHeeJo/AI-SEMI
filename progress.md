...
- 직전 윈도우 추정치 ±radius로 탐색범위 제한(회전의 연속성 제약 추가): radius=25~28에서 **평균 10.8~11.0° / 최악 59.4°** -- 지금까지 중 최고 기록. 단 radius=10/40처럼 스윗스팟을 벗어나면 최악값이 141~145°까지 튀는 민감한 하이퍼파라미터(radius=14~28 구간은 매끄럽게 개선되다가 30~40 구간에서 무너지는 것으로 보임, 정확한 붕괴 지점은 추가 스윕 필요).
...
| **contrast-max(radius=25~28)** | **10.8~11.0°** | **59.4°** |

**미해결**: (1) radius 30~40 사이 붕괴 지점 정확히 특정 안 됨, (2) 이 window/radius 탐색은 여전히 "1024개 후보 전수 평가"라는 계산량이 남아있어 RTL화하려면 더 가볍게 만들 방법(coarse-to-fine 등)이 필요, (3) 아직 고정소수점판/RTL 없음 -- 소프트웨어 레벨 탐색만 완료.
