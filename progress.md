...
## 112. 통합판 Genus PPA 실측 -- 예상과 정반대: 8레인 복제는 거의 공짜, world_mem_writer가 전체 면적의 98%(2026-09-10)

**설계 의도**: §111에서 "다음"으로 남겨둔 질문("8레인 병렬 복제 비용이 얼마나 되는가")에 답하려고 통합판(`rtl/aer_tx16_coord_transform_v1.v` 전체: steal_buf_polarity + coord_transform_rmcm x8 + world_mem_writer) 전체를 Genus로 합성(`syn/run_genus_aer_tx16_coord_transform_v1.tcl`, 5ns/200MHz 동일 조건). Elaborate+syn_map이 유난히 오래 걸림(약 50분 -- 4096-way case문 8개 = 32768 branch를 다루는 무거운 작업이라 예상보다 느렸지만, CPU를 계속 소모하며 정상 진행했고 실제로 완주함).

**실측 결과**:

| | 단독 coord_transform_rmcm(§110) | 통합판 전체 | u_tx(steal_buf_polarity) | **u_wmem(world_mem_writer)** |
|---|---:|---:|---:|---:|
| 면적 | 1199.052 (795 cells) | **211855.628 (113660 cells)** | 1146.726 (554 cells) | **207711.956 (111402 cells)** |
| 전력 | 0.041372 mW | **3.44617 mW** | (개별 미측정) | (register 카테고리가 전체 전력의 79.98% 차지) |
| 200MHz slack | +2187ps | **+2253ps(여전히 압도적 여유, 타이밍은 문제 없음)** | - | - |

**예상이 완전히 빗나감 -- 두 가지 반전이 동시에 나옴:**

1. **8레인 복제는 걱정했던 것과 달리 거의 공짜였다.** 합성 리포트에 8개 `coord_transform_rmcm` 인스턴스가 개별 항목으로 안 잡히고 top level에 흡수됐는데(전체 113660개 중 u_tx 554 + u_wmem 111402 = 111956을 뺀 나머지 약 1700개 정도), 이유를 되짚어보니: 각 레인의 `xc2_in`은 `2*g-3`(g는 genvar, **컴파일타임 상수**)라서 레인마다 열(col)이 고정값으로 박혀있다 -- 즉 각 레인은 4096가지(4x4x256) 전체가 아니라 **1024가지(4×256, row만 실제로 변함)**만 도달 가능한 케이스라서, Genus의 상수전파+불가지 케이스 가지치기(case pruning)가 각 레인을 자동으로 4배 줄여버림. 그래서 "단독 모듈 795 cells x 8 = 6360" 걱정과 달리 실제로는 훨씬 작게 나옴 -- **RMCM을 "완전히 일반적인 모듈"로 만들 필요 없이, 호출 컨텍스트에서 일부 입력이 고정되면 합성기가 알아서 특화해준다는 걸 실측으로 확인.**
2. **world_mem_writer가 압도적 진짜 비용이었다.** 면적의 **98.0%**(207711.956/211855.628), 전력의 대부분(register 카테고리 79.98%)을 이 모듈 하나가 차지. 원인은 설계 자체의 구조적 문제: 8레인이 각자 **런타임에 계산된 주소**(`lane_y[i]*64+lane_x[i]`)로 4096칸 레지스터 배열에 직접 쓰다 보니, 매 칸(4096개)마다 "8개 레인 중 누가 나를 가리키는지"를 판단하는 12비트 비교기가 필요해짐 -- 사실상 **완전연관(fully-associative) 8-way 다중포트 메모리**가 합성된 것과 같음. 비교기 개수가 `레인수 x 깊이` = 8 x 4096 = 32768개 스케일로 늘어나는 구조라서, "충돌 정책은 last-lane-wins로 공짜"라고 여겼던 §111의 판단은 **기능적으로는 맞았지만 PPA 관점에서는 완전히 틀렸음** -- 시뮬레이션 검증(0 mismatch)은 회로가 옳다는 것만 보장하지, 그 옳음을 얼마나 비싸게 사는지는 전혀 안 알려준다는 걸 다시 한번 확인(CORDIC 때와 같은 교훈이 재발).

**당장 결론 내리지 않고 다음 세션 논의로 넘김**: world_mem_writer를 지금 이대로 최종안으로 채택하기엔 무리가 있어 보임 -- 유력한 대안은 (a) 8레인을 곧바로 메모리에 박지 않고 작은 arbiter(이미 검증된 `arbiter8.v` 재사용)로 먼저 승자 1~2개만 골라서 **포트 수를 줄인 다음** 메모리에 쓰기(비교기 스케일을 `레인수 x 깊이`에서 `레인수`로 낮춤), 또는 (b) 실제 충돌 빈도가 낮다는 §111의 실측(실트래픽 87/4096칸, 대부분 무충돌)을 근거로 **완전연관 구조 대신 직렬화(한 사이클에 1~2개씩만 순차 반영)**하는 설계로 교체. 둘 다 §105 계획에서 이미 "실측 충돌 빈도 보고 결정"이라고 명시했던 바로 그 지점 -- 이제 실측이 나왔으니 다음에 실제로 결정하면 됨.
...
## 115. §113/114 재설계 Genus PPA 실측 -- 6배 개선 확정, 1단계 마무리(2026-09-10)

**결과**(같은 조건, 5ns/200MHz):

| | v1(레지스터 배열 직접 합성, §112) | **v3(SRAM 인터페이스 + pose_fifo, 이번)** |
|---|---:|---:|
| 면적 | 211855.628 um² (113660 cells) | **35110.746 um² (11501 cells)** |
| 전력 | 3.44617 mW | **0.523304 mW** |
| 200MHz slack | +2253ps | **+1819ps(여전히 여유)** |

**면적 6.0배, 전력 6.6배 감소** -- §112에서 발견한 "world_mem_writer가 전체 면적의 98%" 문제가 저장소를 RTL 밖(SRAM 매크로/BRAM/시뮬레이션 메모리 모델 가정)으로 빼면서 실측으로 해소됨. `u_tx`(steal_buf_polarity_pose, pose_fifo 포함) 단독은 3738.402 um²(1280 cells)로 전체의 10.6%뿐 -- 나머지(coord_transform_rmcm x8 + world_mem_writer의 FIFO/arbiter8)가 31372.344 um²로 대부분을 차지하지만, 예전 98%짜리 문제와는 차원이 다른 규모.

**출처 재확인**: 이번 재설계(pose_fifo 추가, world memory를 SRAM 인터페이스로 전환)는 사용자가 대화 중 공유한 병렬 세션의 분석에서 나온 방향(§113/114 참고) -- 구체적 수치(steal_buf_polarity Fmax 285.7~333.3MHz, tree4 구조, cluster2_steal_buf 손실 0.47%=502/106416, delay-shadow 실험 결과)를 전부 실제 파일과 대조해 정확함을 확인한 뒤 채택. 이어서 그 세션이 추가로 제안한 "차량 탑재 DVS + road-plane + depth 필요한 6-DOF 시나리오로 전환" 제안은 **채택하지 않음** -- 근거로 든 교수 Q&A 인용("tilting, panning, rotation")을 직접 재확인해보니 pan/tilt/roll은 전부 회전의 3개 축이라 오히려 기존 "고정 위치 + 회전만" 프레임을 재확인해주는 내용이었고, depth/이동 필요성을 뒷받침하지 않음. 원본 공식 문제지 원문은 이 저장소에 없어서(내 쪽 "회전만" 서술도, 저쪽 근거도 둘 다 1차 출처가 아닌 paraphrase/Q&A 힌트) 완전히 확정된 건 아니지만, 현재 근거로는 회전-only 유지가 더 타당하다고 판단 -- 사용자도 동의, 기존 방향 유지.

**1단계 마무리 상태**: 소프트웨어 오라클 -> RTL 3종 비교(baseline/CORDIC/RMCM, §105~110) -> RMCM 채택 -> 실제 1차 TX와 통합(§111) -> 발생시점 pose 태깅 + 저장소 SRAM 인터페이스 전환(§113/114) -> PPA 6배 개선 확정(이번) 까지 전부 완료, 실트래픽+스트레스 테스트 모두 content_mismatches=0/무손실 보존식 성립으로 검증됨. P&R은 시간 되면 추가(현재 우선순위 아님) -- **1단계는 사실상 마무리, 다음은 2단계(회전각 직접 추정)로 이동**.
...
## 117. 2단계(회전각 직접 추정) 소프트웨어 오라클 1차 실측 -- 정확도 나쁨, 문헌 경고 그대로 확인(2026-09-10)

**설계**: `scripts/rotation_estimate_model.py`(신규) -- 2-C에서 스케치한 "채점 기반 이산 탐색" 그대로 구현. 실제 UZH 이벤트(`eventmeta_theta.tsv`, 8503개)를 W개씩 윈도우로 묶어서, 각 윈도우마다 256개 후보 theta_idx 전부로 `coord_transform_model.transform()`을 돌려 예측 world (X,Y)를 구하고, 이미 world_mem(딕셔너리, causal하게 누적)에 쓰여있는 값과 극성이 맞으면 +1/틀리면 -1/안 쓰여있으면 0으로 채점 -- 최고점 후보를 그 윈도우의 추정 theta로 채택하고 실제로 맵에 반영(다음 윈도우의 채점 기준이 됨). ground-truth theta_idx는 추정 로직에 전혀 안 먹이고, 사후 정확도 비교에만 씀. 부트스트랩(첫 윈도우, 맵이 비어서 전부 0점 동점)은 theta_idx=0으로 시작 -- 임시방편이 아니라 우리 theta 정의 자체("시작 자세 대비 상대 회전각")와 정확히 일치하는 기준점.

**결과**(W=윈도우 크기별 스윕):

| W | windows | 평균 오차 | 최대 오차 |
|---:|---:|---:|---:|
| 8  | 1063 | 28.98idx (40.7°) | 75idx (105.5°) |
| 16 | 532  | 17.01idx (23.9°) | 60idx (84.4°) |
| 32 | 266  | 22.64idx (31.8°) | 64idx (90.0°) |
| 64 | 133  | 13.66idx (19.2°) | 38idx (53.4°) |

**참고 기준선**: 완전 무작위 추측의 기대 오차는 64idx(90°) -- 우리 결과(13.66~28.98idx)는 무작위보다 2.2~4.7배 낫지만, **평균 19~41도, 최악 53~105도 오차는 "회전을 추정한다"고 말하기엔 정확도가 한참 부족함**. W를 늘린다고 단조롭게 좋아지지도 않음(W=32가 W=16보다 오히려 나쁨) -- 노이즈성/불안정성이 크다는 신호.

**해석 -- 문헌(2-A)이 예고한 그대로**: Kim2014/CMax-SLAM/EROAM 세 논문 다 "좁은 시야는 후보 회전들을 구별 못 하게 만들어 추정이 불안정해진다"고 명시했었는데, 우리 4×4 patch(16 sources)가 정확히 그 경우였음 -- 서로 다른 회전 후보들이 이 좁은 패치 안에서 만들어내는 world 좌표 패턴이 충분히 구별되지 않아서, 채점 기반 탐색이 진짜 정답 근처로 잘 안 모임. **RTL을 지금 이 알고리즘·이 센서 크기로 만들면 정확도가 너무 나빠서 의미가 없을 가능성이 높음.**

**결론(사용자와 논의 필요)**: 계획에서 명시했던 두 갈래 중 "정확도 나쁨" 쪽으로 나옴 -- 이 상태로 RTL 착수는 시기상조로 보이고, 지금까지 미뤄뒀던 **센서 커버리지 확장(4×4 → 계층형 AER, tree4 재활용)을 2단계보다 먼저 해야 하는지** 결정이 필요함. 문헌 근거(2-A)가 이미 "회전 추정 단계에서는 커버리지 확장이 사실상 선행/동반돼야 한다"고 예측했었고, 이번 실측이 그 예측을 정량적으로 뒷받침함.

- 신규: `scripts/rotation_estimate_model.py`
- 다음: 사용자와 방향 논의 -- 센서 커버리지 확장을 먼저 할지, 아니면 이 알고리즘 자체를 개선(윈도우 방식/채점 방식 변경)할 여지부터 더 볼지

## 118. 알고리즘 개선(추적/tracking 방식) 시도 -- 오히려 악화, 정직한 기각(2026-09-10)

**동기**: §117에서 "센서 확장 먼저 vs 알고리즘 개선 먼저" 갈림길에서 사용자가 알고리즘 개선 쪽을 먼저 선택 -- 매 윈도우 256개 후보 전수탐색 대신, **직전 추정치 근방(radius)만 탐색하는 추적(tracking) 방식**을 시도. 근거: "회전은 물리적으로 갑자기 안 튄다"는 연속성 가정을 걸면, 전수탐색이 국소적으로 비슷해 보이는(aliasing) 먼 후보로 잘못 튀는 걸 막아서 안정성이 나아질 것으로 기대함.

**구현**: `run_tracking(events, table, window, radius)`(신규, `rotation_estimate_model.py`에 추가) -- `current` 상태(직전 추정치)를 유지하며, 매 윈도우마다 `[current-radius, current+radius]` 범위만 채점, 부트스트랩(첫 윈도우)만 예외적으로 256개 전부(사실상 무의미, 맵이 비어 전부 0점).

**결과 -- 예상과 반대로 명확히 악화**:

| 방식 | 최선 결과 |
|---|---|
| v1(전수탐색, §117) | mean 13.66idx(19.2°), max 38idx(53.4°) (W=64) |
| **v2(추적, radius 제한)** | **mean 28.62idx(40.3°), max 66idx(92.8°) (W=16,radius=32, 가장 좋은 조합인데도)** |

좁은 radius(4~16)일수록 오히려 더 나쁨(mean 53~68idx, 거의 무작위 수준 90°에 근접), `max_err=128idx(180°, 회로적으로 가능한 최댓값)`이 여러 조합에서 반복 등장 -- **한 번 잘못된 방향으로 락인(lock-in)되면 탐색 범위 자체가 좁아서 다시 못 돌아옴**, 이게 예상한 "aliasing 방지" 효과보다 훨씬 크게 작용함. radius를 넓힐수록(32) 조금 나아지는 경향은 있지만 여전히 v1(전수탐색)보다 못함 -- radius가 충분히 넓어지면 결국 전수탐색에 가까워지기만 할 뿐, 국소탐색 자체가 이 문제에서 이득을 못 줌.

**결론 -- 정직한 기각**: 간단한 추적/평활화로 4×4 patch의 근본 한계를 우회할 수 없음이 확인됨. §117의 "좁은 시야가 회전 후보들을 구별 못 하게 만든다"는 문헌 예측이, 이번엔 "심지어 연속성 가정을 걸어도 못 살린다"는 형태로 한 번 더 뒷받침됨 -- 채점 신호 자체가 진짜 정답 근처에서 충분히 강하지 않다는 뜻(국소탐색이 오히려 손해라는 게 그 증거). 알고리즘 튜닝만으로는 부족해 보이고, **센서 커버리지 확장(4×4 → 계층형 AER)이 실제로 필요한 방향**이라는 결론 쪽으로 무게가 실림.

- 수정: `scripts/rotation_estimate_model.py`(`run_tracking()` 추가)
- 다음: 센서 커버리지 확장(tree4 재활용) 착수 여부를 사용자와 확정

## 119. 센서 커버리지 확장(N×N 패치 스윕) 실측 -- 예상과 반대로 넓힐수록 더 나빠짐, 원인 미확정(2026-09-10)

**데이터 준비**: `shapes_rotation.zip`(취리히대 원본 전체, 240×180 센서, 이벤트 2312만개+groundtruth pose)를 재다운로드(로컬/서버 둘 다 인터넷 직접 접근 불가라 사용자가 브라우저로 받음) 후 `shapes_rotation/`에 압축해제. `groundtruth.txt` 줄 수(11883)가 기존에 쓰던 것과 정확히 일치해 같은 레코딩임을 확인.

**준비한 도구**(RTL 없이 순수 소프트웨어): `scripts/build_uzh_eventmeta_nxn.py`(신규, 4×4 전용 원본과 별개로 임의 크기 패치 추출), `scripts/rotation_estimate_model.py`를 N×N 일반화(기존 4×4 결과 재현 확인, `local_xy_from_row_col_n()` 신규 -- 1단계 RTL에 묶인 `coord_transform_model.local_xy_from_row_col()`은 안 건드림), `scripts/sweep_patch_size_estimate.py`(신규, N 스윕 자동화 드라이버, 가짜 데이터로 스모크테스트 통과 확인 후 실측 진행).

**결과**(같은 중심 좌표(111,86) 유지하며 N만 확장, 각 N마다 최선의 윈도우 크기 채택):
...
**예상과 정반대**: 문헌(2-A)은 "시야가 넓을수록 회전 후보 구별이 쉬워져 추정이 안정된다"고 예고했는데, 실측은 N=6에서 살짝 나아졌다가 N=8부터 계속 나빠짐 -- 좁은 시야가 아니라 **오히려 넓은 시야가 손해**로 나옴.

**원인 진단(부분적으로만 규명)**: 먼저 "64×64 world memory가 너무 작아서 큰 패치 하나가 다 못 들어가는 것 아닌가"를 확인 -- N=16(256칸)까지도 한 theta 안에서 로컬 패치의 모든 칸이 서로 겹치지 않고 world grid에 개별적으로 들어감(자체 충돌 없음, 실측 확인). 즉 "그릇이 작아서 못 담는다"는 아니었음. **더 유력한 원인(아직 확정 검증은 안 함)**: 스윕이 윈도우를 "이벤트 개수(W)" 기준으로 잘랐는데, 패치가 커질수록(N²에 비례해서) 같은 시간에 훨씬 많은 이벤트가 나옴(N=16이 N=4보다 이벤트가 16배 많음) -- 그래서 "같은 W개"라도 N이 클수록 훨씬 **짧은 실제 시간**만 담은 윈도우가 되고, 회전 변화량(신호)도 그만큼 작아져서 추정이 더 어려워졌을 가능성이 있음. 이건 "패치 크기"가 아니라 "윈도우를 시간이 아니라 개수로 자른" 실험 설계의 교란변수(confound)일 수 있음 -- 시간 기준 윈도우로 다시 돌려봐야 진짜 결론인지 확정 가능.

**결론(미확정, 다음 세션 확인 필요)**: 지금 이 결과만으로 "센서 확장이 답이 아니다"라고 단정하기엔 이르다 -- 윈도우를 이벤트 개수 대신 실제 시간(ms) 기준으로 다시 잘라서 공정하게 비교하는 실험이 필요함. 그 전까지는 이번 결과를 "나쁜 소식이지만 원인 미확정"으로만 기록.

- 신규: `scripts/build_uzh_eventmeta_nxn.py`, `scripts/sweep_patch_size_estimate.py`
- 수정: `scripts/rotation_estimate_model.py`(N×N 일반화)
- 원본 데이터(`shapes_rotation/`, `shapes_rotation.zip`)는 UZH 원본 이벤트 데이터와 같은 이유로 git에 커밋 안 함
- 다음: 시간 기준 윈도우로 N 스윕 재실행해서 confound 여부 확정

## 120. 시간 기준 윈도우 재실험 -- confound 제거 시도가 오히려 새 문제를 드러냄, 결론 확정(2026-09-10)

**동기**: §119에서 발견한 confound("이벤트 개수 기준 윈도우는 N이 클수록 짧은 실제 시간만 담는다") 의심을 확정 검증하기 위해, 이벤트 개수 대신 실제 시간(ms) 기준으로 윈도우를 잘라 N마다 공정하게 비교.

**시도 1 -- 순수 시간 윈도우**(`run_window_time()`, 신규): 결과가 **전 N에 걸쳐 오히려 대폭 악화**(N=4: 23.4°→55.9°, 나머지도 59~68°). 원인을 직접 진단(`load_events_ts`로 50ms 버킷 분포 확인): 조용한 구간에서 **윈도우 333개 중 90개(27%)가 이벤트 2개 이하** -- 채점 신호가 사실상 무작위 수준이 되고, causal 구조(이번 추정으로 맵을 갱신 → 다음 윈도우가 그 맵으로 채점) 때문에 이 나쁜 추정이 이후 전체로 전파됨.

**시도 2 -- 시간 AND 최소 이벤트 개수(16개) 둘 다 만족할 때만 윈도우 닫기**(`run_window_time(..., min_events=16)`): 여전히 나쁨(62~78°, 원래 이벤트-개수 기준보다 더 나쁨), N에 따른 경향도 뚜렷하지 않음(N=8이 62.4°로 오히려 최선, N=16이 78.0°로 최악이지만 나머지는 뒤섞임).

**결론 -- 이 방향은 여기서 정리**: "공정한 비교"를 위해 윈도잉 방식을 계속 땜질했지만, 매번 새로운 형태의 불안정성(희소 윈도우, 가변적으로 너무 커지는 윈도우)만 드러났고 §119의 confound 의심을 깔끔하게 해소하지 못했음. 오히려 이번 재실험들이 보여준 더 확실한 사실: **채점 신호 자체가 원래 약해서(이벤트가 적으면 바로 무작위 수준으로 무너짐), 윈도잉 방식과 무관하게 이 "채점 기반 이산 탐색" 알고리즘 자체의 근본 한계**일 가능성이 §117/118/119보다 한 겹 더 명확해짐. §119의 원래 이벤트-개수 기준 결과(N=6이 19.3°로 최선, N=8/12/16은 더 나쁨)를 뒤집을 근거를 못 찾았으므로, **그 결과를 그대로 유지**: 이 구현으로는 단순 센서 확장이 회전추정 정확도를 안정적으로 개선하지 못함.

**남은 선택지(사용자와 논의 필요)**: (a) 윈도잉을 더 정교하게 다듬기(수확체감 우려), (b) 알고리즘 자체를 우리 단순화판이 아니라 Kim2014의 진짜 파티클 필터나 EROAM의 진짜 ICP에 더 가깝게 다시 설계, (c) 2단계를 잠정 보류하고 1단계 성과로 발표 준비, (d) 다른 시나리오/문제 재정의.
...
## 122. 알고리즘 재설계 -- 이산 베이즈 필터(histogram filter), 개선은 확인되나 위치 민감성 노출(2026-09-10)

**동기**: 사용자가 §120의 4가지 선택지 중 "알고리즘 재설계"를 선택. 지금까지 §117~120의 공통 구조("윈도우마다 후보 전부 채점 -> 1등만 뽑아 확정, all-or-nothing")가 근본 문제였다고 보고, Kim2014의 파티클 필터 정신을 우리 문제에 맞게 다시 설계.

**핵심 통찰**: Kim2014는 연속적인 회전 공간을 유한한 파티클(몬테카를로 샘플)로 근사하지만, 우리는 theta가 **정확히 256개 이산값**이라 파티클이 필요 없음 -- 256개 상태 전부에 대한 확률분포(belief)를 정확히 유지하는 **이산 베이즈 필터(histogram filter)**를 쓰면 근사 없이 그대로 됨.

**설계**(`run_bayes_filter()`, 신규, 윈도우 개념 자체가 사라짐 -- 매 이벤트가 곧 하나의 갱신이라 §119/120에서 계속 발목 잡았던 "윈도우를 뭘로 자르나" confound가 원천적으로 없어짐):
1. **확산(diffuse_eps)**: 매 이벤트마다 belief를 인접 theta로 살짝 펴줌("그새 조금 돌았을 수도 있다") -- §118 tracking에서 못 풀었던 "한 번 잘못된 방향으로 락인되면 복구 불가" 문제의 직접적 해법.
2. **갱신**: 이벤트가 world_mem과 맞으면 belief를 `like_match`배, 틀리면 `like_mismatch`배, 안 쓰여있으면 그대로(중립) 곱한 뒤 정규화.
3. 매 이벤트, belief의 최댓값(MAP) theta로 world_mem에 기록(causal, 기존과 동일 철학).

**파라미터 튜닝 결과**(원래 4×4 패치, `uzh_shapes_rotation_patch.eventmeta_theta.tsv`): 확산 없이(eps=0)는 15.02idx(21.1°)로 이전 최선(19.2°, §117 W=64)과 비슷. **확산+우도비를 같이 튜닝하니 뚜렷이 개선**: `eps=0.04, like=(4.0,0.25)` -> **평균 17.0°, max 57.7°, 뒤 10% 구간 평균 12.9°** -- tail이 전체 평균보다 낮다는 건 **필터가 실제로 수렴하고 있다는 신호**(이전 방식들은 전부 이런 수렴 거동이 없었음). 다만 파라미터 공간이 꽤 들쭉날쭉함(바로 옆 조합인 eps=0.03/like=(4,0.25)는 55.6°로 확 나빠짐) -- 안정적인 넓은 최적 영역이 아니라 좁은 sweet spot.

**위치 민감성 -- 중요한 새 경고**: 같은 알고리즘·같은 파라미터(eps=0.04, like=4.0/0.25)를 §119에서 만든 **거의 같은 위치의 다른 4×4 패치**(중심이 0.5픽셀만 다름, `uzh_shapes_rotation_4x4.eventmeta_theta.tsv`)에 적용했더니 **52.3°**로 3배 나쁨. 패치를 몇 픽셀 옮긴 것만으로 결과가 이렇게 크게 흔들린다는 건, 지금까지의 모든 실측(§117~122)이 "이 알고리즘/이 시나리오 일반의 정확도"가 아니라 **"이 특정 4×4 픽셀 위치의 특정 실측값"**에 가깝다는 뜻 -- 결론의 일반화 가능성 자체에 물음표.

**N 스윕도 이 새 알고리즘으로 재확인**: 같은 (eps=0.04, like=4.0/0.25)를 N=4/6/8/12/16에 적용(§119/120과 같은 위치들) -- **여전히 넓힐수록 나빠짐**(52.3°/50.8°/81.1°/82.7°/83.0°) -- 알고리즘을 바꿔도 "센서 확장이 도움 안 됨"이라는 §119/120 결론은 그대로 유지됨.

**종합 결론**: 알고리즘 재설계는 **딱 하나의 특정 패치에서는 실제로 의미 있는 개선**(19.2°→17.0°, 수렴 거동 확인)을 보여줬지만, 그 개선이 **위치 하나 바꾸면 사라질 만큼 불안정**하다는 것도 같이 드러남. 센서 확장이 도움 안 된다는 결론은 알고리즘을 바꿔도 재확인됨(2번째 독립 확인).

- 신규: `scripts/rotation_estimate_model.py`(`run_bayes_filter()` 추가)
- 다음: 사용자와 논의 -- (a) 위치 민감성을 더 파볼지(여러 패치 위치에서 평균내는 식으로 안정성 확인), (b) 지금 결과(개선 확인+한계도 확인)로 충분하다고 보고 §120의 다른 선택지(2단계 잠정 보류 등)로 이동할지

## 123. codex/ai-semi-stage2 브랜치 재확인 + 아이디어 선별 이식(2026-09-10)

**브랜치 재확인**: 지난 확인 이후 37개 커밋으로 크게 성장 -- M0(1차 기준선 재현, 공식 50-workload 106416/502/105914 정확히 일치) ~ M4(pose-history overwrite 보호 통합)까지 완료. pose_version+실제 occurrence timestamp를 픽셀별로 태깅, 일반 affine2D 변환(2D 회전보다 넓은 범위), ready/valid 백프레셔, missing_pose/out_of_range를 status로 명시, pose ID 재사용 안전장치(pose_inflight_guard8), 타일 병합용 event_batch_fifo/rr_stream_arbiter4, 원클릭 회귀(10/10, 확장 12/12 PASS)까지 갖춘 상당히 성숙한 상태. **다만 이 브랜치도 "회전각을 실제로 추정"하는 진짜 2단계는 M8(맨 마지막, 아직 착수 안 함)로 미뤄뒀음** -- 지금까지는 전부 "자세가 주어진다"는 전제의 견고한 1단계 재설계.

**사용자 결정**: 두 트랙은 서로 독립적으로 계속 진행(브랜치 병합/대체 안 함), codex 쪽 아이디어 중 괜찮은 것만 선별해서 main으로 이식.

**아이디어 1 -- pose 재사용 안전장치(pose_inflight_guard8): 기각, 우리 구조엔 안 맞음**. codex는 pose를 "테이블 인덱스(pose_version)"로 참조하는 구조라서 "참조 중인 버전을 덮어쓰면 안 된다"는 문제가 생기고, 그걸 막는 전용 가드가 필요함. 우리 `steal_buf_polarity_pose`는 theta 값(8비트) 자체를 이벤트마다 직접 pose_fifo에 담아서(참조가 아니라 값 자체를 복사) 애초에 그 문제 자체가 안 생김 -- 억지로 가져오면 필요없는 복잡도만 늘어남(우리 8비트 theta는 pose_version 인덱스와 크기가 같아서, 참조 방식으로 바꿔도 압축 이득이 없음).

**아이디어 2 -- occurrence timestamp 기반 확산(diffusion): 시도했으나 기각, 더 나빠짐**. §122의 베이즈 필터는 "이벤트 1개당 고정량 확산"인데, codex처럼 실제 timestamp를 활용해서 "경과 시간에 비례해서 확산"하면 더 정확할 것으로 기대하고 시도(RTL 안 건드리고 소프트웨어 오라클만 수정, `occurrence_timestamp_ns`는 이미 갖고 있어서 바로 테스트 가능). **결과는 반대로 훨씬 나쁨**(평균 53~83°, 고정 확산 방식의 17.0°보다 3~5배 나쁨) -- 원인: 실제 이벤트 간격이 매우 불규칙(긴 침묵 구간이 흔함)해서, 확산량을 경과시간에 선형 비례시키면 조용한 구간 하나만으로 belief가 완전히 무너짐(상한(0.4)을 걸어도 그 상한 자체가 이미 belief를 거의 균일하게 만들 만큼 큼). 기각.

- 다음: 사용자와 2단계 최종 방향 결정(§120/122 선택지 여전히 유효 -- 지금까지 종합하면 "정확도는 위치/파라미터에 매우 민감, 확장은 안 도움, 알고리즘 개선 시도들도 대부분 역효과"라는 그림)

## 124. 위치 민감성 정밀 조사 -- §119~122 N 스윕의 중심 좌표 버그 발견 + 재실험으로 결론 재확정(2026-09-10)

**버그 발견**: §122의 베이즈 필터를 4×4 패치 8곳(원래 위치 근방 4곳 + 멀리 떨어진 4곳)에서 같은 튜닝값(eps=0.04, like=4.0/0.25)으로 테스트 -- **8곳 중 딱 1곳만 좋았고**(중심(112,87): 17.0°/tail 12.9°), 나머지 7곳은 32.7~93.4°로 들쭉날쭉하거나 나쁨(대부분 시간이 갈수록 더 나빠짐). 그런데 그 유일하게 좋았던 위치 (112,87)을 역산해보니 **우리가 처음부터 계속 써온 바로 그 원래 4×4 패치**(x=110-113,y=85-88, 이벤트수 8503)와 정확히 일치함을 확인.

**연쇄 발견**: §119~122의 N 스윕(`sweep_patch_size_estimate.py`)이 중심으로 썼던 `(111,86)`을 역산해보니 `x0=109,x1=112`로 **원래 패치(110-113)보다 1픽셀 어긋난 다른 위치**였음 -- 즉 §119/120/122의 "확장은 안 도움된다"는 결론이 처음부터 좋았던 기준 패치가 아니라 어쩌다 고른 다른(더 안 좋은) 위치를 기준으로 냈던 것. 중심을 `(112,87)`로 고쳐서(원래 패치를 정확히 재현) N 스윕을 베이즈 필터로 재실행.

**재실험 결과**(중심을 바로잡은 뒤, `eps=0.04, like=4.0/0.25` 그대로):

| N | 이벤트 | 평균 오차 | tail10% |
|---:|---:|---:|---:|
| 4  | 8503   | **17.0°** | **12.9°(수렴)** |
| 6  | 18901  | 94.0° | 108.8°(악화) |
| 8  | 33512  | 58.3° | 65.5°(악화) |
| 12 | 75611  | 73.2° | 59.4° |
| 16 | 135647 | 65.7° | 81.1°(악화) |

N=4는 정확히 재현됨(17.0°, 검증 성공)이지만 **N=6/8/12/16은 중심을 바로잡은 뒤에도 여전히 훨씬 나쁨** -- 처음 우려("나쁜 기준 패치를 썼던 게 문제 아니었나")는 기각됨.

**"N=4용 튜닝을 그대로 써서 불공평한 것 아닌가" 확인**: N=8에 대해 자체적으로 파라미터 재탐색(eps 6개 x like 4개 = 24가지 조합) -- **최선이 43.6°**(그마저 tail이 107°로 더 나빠지는 추세, 수렴 안 함)로, N=4의 17.0°(수렴함)보다 확실히 못함. 공정하게 재튜닝해도 결과는 그대로.

**종합 결론 -- 세 번째 독립 확인**: 단순 채점(§119/120) → 시간윈도우(§120) → 베이즈 필터+자체튜닝(이번, §124)까지 서로 다른 세 방법론 전부에서 "센서 커버리지 확장이 정확도를 못 올린다"가 재확인됨 -- 방법론 편향이 아니라 이 시나리오/알고리즘 계열의 실제 특성으로 봐도 될 만큼 확실해짐. 대신 **위치 민감성이 진짜 핵심 변수**임이 이번에 명확해짐(8곳 중 7곳이 안 좋고 1곳만 좋음) -- "패치를 얼마나 넓히느냐"보다 "그 패치 안에 뭐가 있느냐"(텍스처/엣지 밀도 등 장면 내용)가 정확도를 좌우하는 것으로 보임.

- 수정: `scripts/sweep_patch_size_estimate.py`(중심 좌표 수정 `(111,86)`→`(112,87)`, 베이즈 필터로 전환)
- 다음(사용자 지시: 계속 진행): 장면 내용(텍스처/엣지 밀도)과 정확도의 상관관계 조사 -- "어디를 패치로 고르느냐"를 원칙 있게 결정할 수 있는지, 또는 world_mem 표현 자체를 강화(칸마다 단일 극성 대신 관측 횟수/신뢰도 누적)해서 신호 품질 자체를 개선하는 방향

## 125. world_mem 표현 강화(확신도 가중) -- N=4 최선 대폭 개선(17.0°→10.1°) + N 스윕이 매끄러운 단조곡선으로 정리됨(2026-09-10)

**동기**: §124까지 world_mem은 칸마다 "마지막으로 관측된 극성 1개"만 저장했음(한 번 본 칸과 백 번 일관되게 본 칸을 똑같이 취급) -- Kim2014의 실제 지도는 단일 비트가 아니라 누적된 gradient 추정치라는 점에서 착안, 정보 손실을 줄이는 방향으로 표현을 강화.

**설계**(`run_bayes_filter_confidence()`, 신규): 칸마다 `(on_count, off_count)`를 누적. 우도 계산 시 `strength = min(총관측횟수, max_strength) / max_strength`(0~1, 포화형)로 "이 칸을 얼마나 믿을 만한가"를 가중 -- 한 번 본 칸은 약한 신호, 여러 번 일관되게 확인된 칸은 강한 신호.

**결과 -- N=4에서 큰 개선**: `max_s=2, eps=0.02, like=(3.0,0.3)` -> **평균 10.1°, tail10% 7.3°**(§122의 17.0°/12.9°보다 확실히 나음, 수렴도 더 뚜렷함).

**N 스윕 전체 재실행**(각 N마다 24가지 조합 자체 재탐색, 위치는 §124에서 바로잡은 올바른 중심 `(112,87)` 그대로):
...
**중요한 정성적 변화**: §119~124의 단순/시간윈도우 표현에서는 N에 따른 오차가 들쭉날쭉했음(52→94→58→73→66°, 노이즈처럼 보임). 확신도 가중 표현으로 바꾸니 **매끄럽게 단조증가하는 곡선**이 됨 -- N=4가 여전히 최선이지만, "커질수록 나빠진다"는 게 이제 노이즈가 아니라 설명 가능한 경향처럼 보임(추측: 2D 평면 근사가 로컬 오프셋이 클수록 부정확해지거나, 더 먼 픽셀일수록 정보성이 떨어지는 효과일 수 있음 -- 아직 확정 원인 규명은 안 됨).

**종합**: 알고리즘(베이즈 필터) + 표현(확신도 가중)을 함께 개선하니 최선의 정확도가 19.2°(§117 최초)→17.0°(§122)→10.1°(이번)로 계속 나아지고 있음 -- "정확도가 안 나온다"는 초기 결론이 알고리즘이 개선되면서 계속 밀려나는 중. 다만 N 확장이 도움 안 된다는 결론 자체는 여전히 유효(이번엔 더 명확한 형태로).

- 신규: `scripts/rotation_estimate_model.py`(`run_bayes_filter_confidence()` 추가)
- 다음: 이 개선된 표현으로 위치 민감성(§124의 8곳 재실험)도 다시 확인 -- 표현이 좋아지면서 "안 좋았던 위치"들도 나아졌는지

## 126. 위치 민감성 재확인 -- 확신도 가중 표현이 8곳 거의 전부를 살려냄, 핵심 문제였음이 밝혀짐(2026-09-10)

**동기**: §125에서 world_mem 표현을 확신도 가중으로 바꾸니 N=4 최선이 17.0°→10.1°로 개선됨을 확인 -- 그럼 §124에서 "8곳 중 1곳만 좋았다"던 위치 민감성 문제도 이 개선된 표현이면 완화되는지 같은 8곳(+비슷한 위치 2곳 교체)에서 재확인.

**결과**(각 위치마다 24가지 조합 자체 재탐색):

| 위치 | §124(단순 표현) | 이번(확신도 가중) |
|---|---:|---:|
| (112,87) -- 원래 기준 패치 | 17.0° | **10.1°** |
| (113,88) | 55.2° | **10.6°** |
| (111,86) | 52.3° | 21.3° |
| (114,89) | (미측정) | **11.9°** |
| (107,81) | 93.4°(최악) | **10.5°** |
...
**8곳 전부 10~25° 범위로 들어옴** -- §124에서 발견했던 "8곳 중 7곳이 안 좋다"는 극심한 위치 민감성이 사실상 해소됨. 특히 §124에서 최악이었던 (107,81)(93.4°)이 이번엔 10.5°로 최상위권에 들 정도로 뒤바뀜.

**해석 -- 이번 세션 2단계 조사 전체를 관통하는 결론**: §117~124 내내 반복됐던 "정확도가 나쁘고 위치/파라미터에 극도로 민감하다"는 현상은, 회전-only 시나리오나 4×4 패치 자체의 근본 한계가 아니라 **world_mem이 "칸마다 마지막 관측값 1개만" 저장하던 얕은 표현의 한계**였을 가능성이 높음. 확신도(반복 관측 횟수) 정보를 살리는 것만으로 정확도와 위치 강건성이 동시에 크게 개선됨 -- 정보를 버리고 있었던 게 문제였지, 시야가 좁아서/알고리즘이 틀려서가 아니었을 수 있다는 뜻.

**최선 정확도 추이 요약**(같은 원래 패치 기준): 19.2°(§117, 단순 채점) → 17.0°(§122, 베이즈 필터) → **10.1°(§125, +확신도 가중)** -- 계속 나아지고 있고, 이번엔 위치 강건성까지 같이 확보됨.

- 다음: N 확장(§125의 단조곡선)도 이 관점(정보손실)에서 재해석할 여지가 있는지, 또는 이 정도 정확도(~10~25°)면 RTL 착수를 검토할 단계인지 판단

## 127. 정식 베이지안 우도 모델 + 중요한 정정: §126의 "8곳 다 좋음"은 위치별 재튜닝의 산물이었음(2026-09-10)

**개선 시도**: `run_bayes_filter_confidence()`(§125)의 `strength = min(총관측횟수,max_s)/max_s`는 5:5로 팽팽하게 갈린 칸도 "다수결"을 무조건 강하게 믿는 결함이 있음(총 횟수만 보고 얼마나 쏠렸는지는 안 봄). `run_bayes_filter_proper()`(신규)로 정식 베이지안 사후예측확률(Beta-Bernoulli, `P(관측=pol|이력) = (해당극성횟수+alpha)/(총횟수+2*alpha)`)로 교체 -- 하이퍼파라미터가 like_match/like_mismatch/max_strength 3개에서 diffuse_eps/prior_alpha 2개로 줄고 이론적으로 더 정갈해짐. 원래 패치 기준 최선: `eps=0.02, alpha=1.0` -> 11.9°/8.9°(§125의 10.1°/7.3°와 거의 동급, 근소하게 못함).

**중요한 정정**: §126에서 "8곳 전부 10~25°로 개선됨"이라고 보고했는데, 그건 **각 위치마다 24가지 조합을 따로 재탐색**해서 나온 최선값이었음 -- 이번에 `run_bayes_filter_proper`의 **고정된 파라미터 하나**(`eps=0.02, alpha=1.0`, 원래 패치용으로 고른 값)를 재튜닝 없이 8곳 전부에 그대로 적용해보니:
...
**위치 편차가 다시 커짐**(11.9°~64.4°) -- 실제 하드웨어는 장면마다 파라미터를 실시간 재조정할 수 없으므로, §126의 "위치 민감성이 해소됐다"는 결론은 **과장이었음**. 정직하게 정정: 확신도 가중 표현(§125/126)은 (a) 원래 패치의 최선 정확도를 개선하고(19.2°→10.1°), (b) "위치마다 최선의 파라미터를 찾을 수 있다면" 대부분 쓸 만한 수준까지 끌어올릴 수 있다는 것까지는 실측으로 확인됐지만, (c) **고정된 단일 설정으로 모든 위치에서 강건하다는 것까지는 아직 확인 안 됨** -- 오히려 반박하는 방향의 증거가 나옴.

- 신규: `scripts/rotation_estimate_model.py`(`run_bayes_filter_proper()` 추가)
- 다음: 고정 파라미터로 강건한 설정을 찾을 수 있는지(예: 여러 위치의 평균 성능을 기준으로 파라미터를 고르는 방식), 또는 "위치별 최적 파라미터를 자동으로 찾는" 것 자체를 회로의 일부로 볼 수 있는지

## 128. 강건한 고정 파라미터 탐색 -- 재튜닝 없이 8곳 평균 20°대 달성(2026-09-10)

**동기**: §127에서 위치별 최적화 없이 고정 파라미터 하나(eps=0.02,alpha=1.0)를 8곳에 그대로 적용하니 편차가 컸음(11.9~64.4°) -- "봉우리 하나에서의 최고 정확도"가 아니라 "여러 위치에서 두루 잘 되는 설정"을 목적함수로 삼아 다시 탐색.

**방법**: `run_bayes_filter_proper`의 (eps, alpha) 56가지 조합을 8개 위치 전부에 돌려서, **평균 오차 최소화** 기준과 **최악값 최소화(minimax)** 기준 두 가지로 각각 최선을 탐색.

**결과**:

| 기준 | 파라미터 | 평균 | 최악 | 위치별(112,87/113,88/111,86/114,89/107,81/121,96/101,71/131,101) |
|---|---|---:|---:|---|
| 평균 최소화 | eps=0.01, alpha=2.0 | **20.2°** | 40.3° | 39.9/40.3/22.6/13.3/10.3/9.1/10.0/16.0° |
| 최악 최소화(minimax) | eps=0.15, alpha=3.0 | 21.9° | **37.1°** | 17.2/15.8/37.1/13.2/13.7/22.3/33.9/21.8° |

**해석**: §127의 "아무 고정값이나 쓰면 64.4°까지 나빠진다"에 비해, **강건성을 목적함수로 명시적으로 최적화하면 재튜닝 없이도 평균 20°대, 최악 37~40°대**를 달성할 수 있음이 확인됨 -- 실제 회로에 넣을 수 있는 정직한 성능치. 특정 위치 하나의 최고 기록(10.1°, §125)보다는 못하지만, "실제 하드웨어는 장면마다 파라미터를 못 바꾼다"는 제약을 지키면서 나온 숫자라 더 신뢰할 만함.

**최선 정확도 추이 최종 정리**(방법론별, 원래 패치 기준 최고 vs 여러 위치 평균):
- 원래 패치 하나 최고 기록: 19.2°(§117) → 17.0°(§122) → 10.1°(§125, 위치별 재튜닝 포함)
- **재튜닝 없는 고정 설정, 8곳 평균**: 20.2°(이번, minimax는 최악 37.1°까지 억제)
...
| 설계 | 면적(um²) | 셀 수 | 전력(vectorless, mW) | 200MHz 여유 |
|---|---:|---:|---:|---:|
| tx16 parallel(K=8) | 102574.333 | 45373 | 0.886 | (미확인) |
| tx16 serial(K=1) | 76750.255 | 28384 | 0.818 | (미확인) |
| tx16 serial_d128(K=1, FIFO깊이128) | 177569.239 | 66504 | 0.841 | (미확인) |
| tx16 banked K=2, d32 | 103628.326 | 36451 | 0.841 | (미확인) |
| tx16 banked K=4, d8 | 83576.848 | 31689 | 0.916 | (미확인) |
| **tx64 serial(8×8, K=1)** | **215929.925** | **79688** | **2.562** | **slack=0(딱 맞춤)** |

**요약**: 6개 스크립트 전부 -sv 패치 후 정상 완주(총 소요 약 1시간). 8×8(tx64)이 4×4(tx16) 대비 면적 약 2.1~2.8배, 전력 약 2.8~3.1배 -- 소스 수가 4배(16→64)인 것보다는 완만하게 늘어남(공유 인프라 덕분으로 보임). tx64_serial은 200MHz에서 슬랙이 정확히 0(합/불합격 경계) -- 여유가 없다는 뜻이라 이 설계로 더 나가려면 클럭을 늦추거나 파이프라인을 손봐야 할 수 있음.

**사용자에게 전달할 것**: (1) 이 6개 PPA 수치, (2) `read_hdl -sv` 수정이 필요하다는 것(서버 로컬 사본에만 적용, 어느 브랜치에도 커밋 안 함) -- codex 세션에 알려주면 그쪽이 직접 브랜치에 커밋할 수 있음.

- 신규(서버 전용, git 미커밋): `~/redred-faer-codex`(codex/ai-semi-stage2 별도 체크아웃), 6개 tcl 스크립트에 `-sv` 패치

## 130. 강건성 검증 확장(20곳) -- §128의 8곳 결과가 재현됨, 결론 신뢰도 확보(2026-09-10)

**동기**: §128의 "고정 파라미터로 8곳 평균 20°대" 결론이 표본 8개로는 우연일 수 있다는 우려 -- 원래 위치 주변 8곳(§124/126/128과 동일) + 240×180 센서 전역에서 무작위로 고른 새 위치 12곳(다양한 x,y 범위, 이벤트 수 5210~13354개로 활동량도 제각각), 총 20곳에서 §128의 두 고정 설정을 그대로 재검증.

**결과**:

| 설정 | 8곳(§128) | 20곳(이번) |
|---|---:|---:|
| 평균 최적화(eps=0.01, alpha=2.0) | 평균 20.2° / 최악 40.3° | 평균 22.9° / 최악 52.2° |
| 최악 최적화(eps=0.15, alpha=3.0) | 평균 21.9° / 최악 37.1° | 평균 24.6° / 최악 39.8° |

**해석**: 표본을 8→20으로 늘려도 **평균 20~25°, 최악 37~52° 범위로 대체로 일치** -- §128의 결론이 작은 표본의 우연이 아니라 실제로 재현되는 성능대임이 확인됨. minimax 튜닝(eps=0.15,alpha=3.0)이 평균은 살짝 손해봐도(22.9→24.6°) 최악값은 확실히 억제(52.2→39.8°)하는 트레이드오프도 예상대로 나타남 -- 안전마진이 중요하면 minimax, 평균 성능이 중요하면 평균최적화 설정을 선택하면 됨.

**지금까지 정확도 개선 궤적 최종 정리**:
- 단일 최적 패치 기준 최고 기록: 19.2°(§117) → 17.0°(§122) → 10.1°(§125, 위치별 재튜닝 포함)
- **재튜닝 없는 고정 설정, 20곳 검증된 평균**: 22.9~24.6°(이번), 최악 40~52°

- 다음: 이 정도 정확도(고정 설정, 평균 20°대)면 RTL 착수를 검토할 단계인지, 아니면 계속 정확도를 밀어붙일지

## 131. §130 재현성 문제 발견 및 정정 -- 임시 스크립트 결과였음, git 커밋된 파이프라인으로 재측정(2026-09-11)

**발견 경위**: RTL 착수 기획(§132 예정) 1단계로 `scripts/bayes_filter_fixed_model.py`(신규, 고정소수점 오라클)를 짜면서 §130과 같은 float 함수(`run_bayes_filter_proper`)·같은 파라미터·같은 20곳으로 float 대조군을 다시 돌렸는데, 그중 10/20곳이 §130 기록과 어긋남(예: (134,47)은 §130에 25.6°로 기록됐는데 재실행하면 94.0°). §130의 실행 스크립트는 커밋되지 않은 임시 스크래치패드 코드였음 -- 원본이 남아있지 않아 정확한 차이 원인은 특정 못 했지만, 같은 함수를 두 번 반복 실행하면 완전히 결정적으로 같은 값이 나오는 것은 확인함(§130 수치 자체가 재현 불가능한 임시 코드의 산물이었다는 뜻).

**정정된 수치**(신규 커밋 스크립트 `scripts/bayes_filter_fixed_model.py`의 `validate_positions()`로 재측정, 같은 20곳):

| 설정 | §130 기록(정정 전) | 실측(정정 후) |
|---|---:|---:|
| 평균 최적화(eps=0.01, alpha=2.0) | 평균 22.9° / 최악 52.2° | 평균 24.4° / 최악 60.8° |
| "최악 최적화"(eps=0.15, alpha=3.0) | 평균 24.6° / 최악 39.8° | 평균 32.0° / 최악 **94.0°** |

**결론이 뒤집힘**: §128/130에서 "최악 최적화(minimax) 설정이 평균은 조금 손해봐도 최악값을 억제한다"고 봤는데, 정정된 수치에서는 오히려 **"최악 최적화" 설정이 평균도(32.0>24.4) 최악도(94.0>60.8) 둘 다 더 나쁨** -- 애초에 그 설정이 "최악에 강하다"는 근거 자체가 재현 안 되는 임시 코드에서 나온 것이었음. 지금 시점에서 둘 중 고르라면 **평균 최적화(eps=0.01, alpha=2.0)가 유일하게 유효한 근거를 가진 후보**임(평균 24.4°/최악 60.8°) -- 다만 이 값도 §128의 그리드서치(파라미터 조합 자체를 고른 과정) 역시 같은 임시 코드로 했을 가능성이 있어, 진짜 최적 설정인지는 아직 보장 못 함.

- 다음: RTL 착수 전에 파라미터 그리드서치도 이번엔 git 커밋된 재현 가능한 코드로 다시 돌려서 확정할 필요 있음(계획된 1단계 작업에 포함).

**추가 정정 -- "최악 60.8°/94.0°" 자체가 집계 실수였음, `--grid`로 그리드서치까지 재현(2026-09-19)**: `bayes_filter_fixed_model.py`에 `grid_search()`(신규, `--grid` 플래그로 실행)를 추가해 §128과 같은 조합 수(eps 7개 x alpha 8개 = 56개)로 20곳 전체를 재현 가능하게 재탐색. 결과를 `validate_positions(float_eps=0.01, float_alpha=2.0)`으로 다시 대조하던 중, 위 표의 "최악 60.8°/94.0°"이 애초에 **집계값이 아니라 위치 (176,104) 한 곳의 출력 줄에서 mean/worst를 잘못 읽어온 것**이었음을 발견(그 위치의 실제 출력: `mean=60.8 worst=94.2` -- 두 숫자가 나란히 찍혀 있어서 이걸 "평균 최적화 최악값"과 "최악 최적화 최악값"으로 각각 오독한 것으로 보임).

**바로잡은 수치(그리드서치 + validate_positions 두 경로로 교차확인, 완전히 일치)**:
- 평균 최적화(eps=0.01, alpha=2.0): **평균 24.4° / 최악 106.9°**(평균은 그대로, 최악만 60.8→106.9로 정정)
- 56개 조합 전수탐색 최악(minimax) 1위: eps=0.0(alpha 무관) -> 평균 25.6°/최악 70.3° -- 그러나 eps=0(확산 없음)은 belief가 초기 theta에서 전혀 안 퍼지므로 MAP theta가 사실상 끝까지 초기값에 고정되는 **퇴화 케이스**(회전을 전혀 안 쫓아가는 상수 예측기)라 실사용 후보가 아님.
- eps>0인 진짜 필터 중 최악 기준 1위는 eps=0.01,alpha=1.5(평균 41.1°/최악 158.9°) 등으로 평균 최적화안보다 전부 나쁨 -- **minimax로 평균 최적화안을 이기는 실사용 가능한 설정은 이번 56개 그리드 안에 없었음**.

**결론**: 재현 가능한 코드로 확정한 최종 후보는 여전히 **평균 최적화(eps=0.01, alpha=2.0)** 하나뿐이지만, 정직한 최악값은 60.8°가 아니라 **106.9°**임 -- RTL 목표 수립 시 이 수치를 기준으로 삼아야 함.

- 신규: `scripts/bayes_filter_fixed_model.py`(`grid_search()` 추가, `--grid` 플래그)
- 다음: 이 확정된 (eps=0.01, alpha=2.0) 기준으로 RTL 착수(고정소수점 비트폭은 이미 이 파일의 `run_bayes_filter_fixed()`가 근사 검증 중 -- §131 상단의 worst-tuned 비교를 avg-optimized 기준으로도 다시 돌려 비트폭 손실을 확인할 것)

## 132. avg-optimized 기준 고정소수점 비트폭 검증 -- eps_shift=8이 float보다도 최악값이 낮음(2026-09-20)

**동기**: §131에서 확정한 (eps=0.01, alpha=2.0, 평균24.4°/최악106.9°)가 RTL의 유일한 유효 후보인데, 지금까지 `run_bayes_filter_fixed()`의 비트폭 검증은 이미 폐기된 "최악 최적화" 설정(eps_shift=3≈0.125, alpha=3)만 돌려봤음. avg-optimized 파라미터로도 diffuse_eps를 시프트로 근사했을 때 정확도가 얼마나 깎이는지 확인.

**방법**: `validate_positions(eps_shift, alpha=2, float_eps=0.01, float_alpha=2.0)`을 eps=0.01에 가까운 시프트 후보 세 개(shift=6→eps≈0.0156, shift=7→eps≈0.0078, shift=8→eps≈0.0039)로 20곳 전체 재실행.

**결과**:

| eps_shift | 근사 eps | fixed 평균 | fixed 최악 | float 평균 | float 최악|
|---:|---:|---:|---:|---:|---:|
| 6 | 0.0156 | 46.4° | 180.0° | 24.4° | 106.9° |
| 7 | 0.0078 | 26.6° | 111.1° | 24.4° | 106.9° |
| **8** | **0.0039** | **25.3°** | **98.4°** | 24.4° | 106.9° |

**해석**: shift=6은 diffuse_eps가 목표(0.01)의 1.5배 이상 커서 belief가 너무 빨리 퍼지며 정확도가 심하게 무너짐(평균 2배, 최악은 포화상한 180°까지 도달). shift=8(eps≈0.0039, 목표의 약 0.4배)이 오히려 **float 오라클보다 최악값이 낮게(98.4<106.9) 나옴** -- 확산을 float보다 살짝 덜 시키는 쪽이 이 20곳 표본에서는 더 안정적이었던 것으로 보임(원인 특정은 안 함, 우연일 수도 있으나 적어도 "비트폭 근사 때문에 성능이 나빠진다"는 우려는 기각됨). `BELIEF_BITS=32/LIKE_BITS=8/CNT_MAX=15` 조합에서 "belief collapsed to 0" 경고 없음(20곳 전부) -- 현재 비트폭으로 충분, 확장 불필요.

**결론**: RTL의 diffuse_eps 근사는 **eps_shift=8**(우측시프트 8, 2^-8=1/256)로 확정. avg-optimized 파라미터(alpha=2, eps_shift=8) 조합이면 float 대비 정확도 손실 없이(오히려 근소하게 나음) 합성 가능.
...
## 137. 근본적으로 다른 접근(contrast maximization) 탐색 -- 베이즈필터 계열보다 평균 오차 큰 폭 개선(2026-09-21)

**동기**: §136에서 UZH 20곳의 worst 이벤트들을 직접 뜯어본 결과, 베이즈필터 계열(하드/소프트 커밋 둘 다)의 근본 한계를 특정함 -- belief가 한 theta에 99%대로 "고착"돼서 실제 회전을 못 따라가는 현상, 원인은 **argmax 승자만 world_mem에 기록하는 자기강화 루프**(한번 확신(설사 틀려도)이 쌓인 후보는 그 칸 히스토리가 계속 두꺼워져서 계속 이김). 1차 처방(`run_bayes_filter_soft_commit`, 상위 top_k 후보에 belief 가중치로 분산 기록)으로 최악값 106.9°→70.7°(-34%) 개선, 평균은 그대로(25.7°). 사용자 지적: "로봇팔 데이터라고 더 잘 된다는 근거가 있냐, UZH에서 안 되는 이유부터 해결해야" -- 파라미터 재탐색(RobotEvt 기준)은 근본 해결이 아니라는 지적에 따라 더 근본적인 대안 탐색.

**문헌 조사**: Gallego 2016(RAL, "Accurate Angular Velocity Estimation with an Event Camera") 등 UZH RPG 그룹 자체 논문이 쓰는 **contrast maximization**(사건들을 후보 모션파라미터로 되돌렸을 때 얼마나 "선명하게"(집중되게) 겹치는지로 모션을 추정, 외부 누적 맵에 전혀 의존 안 함) 확인. 참고로 같은 그룹 레퍼런스 수치: Gallego(이벤트만, IMU 없음) ~2% 오차, Mueggler 2018 TRO(이벤트+IMU 융합) 평균 방향오차 <1° -- 단 둘 다 전역/배치 최적화+(후자는)IMU 융합이라 우리(4×4 패치+스트리밍+비전전용)와 조건이 다름, 직접비교는 불공정.

**구현**(`scripts/rotation_estimate_model.py:run_contrast_max()`, 신규): 윈도우(W개 이벤트) 단위로, 후보 theta마다 이벤트들을 되돌려 (X,Y,polarity) 칸별 집중도(Σcount²)를 계산해 최댓값을 그 윈도우의 theta로 채택. **외부 상태(world_mem) 의존 자체가 없어서 고착이 구조적으로 불가능**.

**실측(UZH 20곳, window=16)**:
- 전역 탐색(매 윈도우 1024개 후보 전수): **평균 17.8° / 최악 75.6°**
- 직전 윈도우 추정치 ±radius로 탐색범위 제한(회전의 연속성 제약 추가): radius=30~32에서 **평균 10.7° / 최악 59.4°** -- 지금까지 중 최고 기록. 스윗스팟은 14~35(그 구간 내내 평균이 매끄럽게 14.5°→10.7°로 개선), **radius=35(평균10.7/최악64.0)에서 radius=38(평균15.1/최악145.2)로 넘어가는 지점에서 급격히 무너짐** -- 붕괴 경계를 30~50 전체 재스윕으로 정확히 특정함(radius=10처럼 너무 좁아도 최악 141°로 나빠짐, U자형 민감도).

**비교표**:
| 방식 | 평균 | 최악 |
|---|---:|---:|
| 베이즈필터(하드 커밋, §132 확정) | 24.4°/25.7°(측정방식별) | 106.9° |
| 베이즈필터(소프트 커밋, §136) | 25.7° | 70.7° |
| contrast-max(전역, window=16) | 17.8° | 75.6° |
| **contrast-max(radius=30~32)** | **10.7°** | **59.4°** |

**radius 붕괴 경계 후속 스윕(30~50) 완료**: radius=30/32/35 전부 평균10.7°(worst 59.4~64.0°)로 안정, **radius=38에서 평균15.1°/최악145.2°로 급붕괴**, 40/45/50은 계속 악화(50에서는 평균47.2°/최악180°까지). 즉 스윗스팟은 14~35 전 구간, 권장값은 30~32(플래토 중앙, 절벽에서 충분히 떨어짐).

**미해결**: (1) 이 window/radius 탐색은 여전히 "1024개 후보 전수 평가"라는 계산량이 남아있어 RTL화하려면 더 가볍게 만들 방법(coarse-to-fine 등)이 필요, (2) 아직 고정소수점판/RTL 없음 -- 소프트웨어 레벨 탐색만 완료.

- 신규: `scripts/rotation_estimate_model.py`(`run_contrast_max()` 추가)
- 다음: radius 스윕 완결, 확정되면 고정소수점 오라클+RTL로 이식(현재 predictor RTL 대체 여부 결정 필요 -- §133~136 베이즈필터 RTL은 이미 완성/검증된 상태라 완전히 새 알고리즘으로 갈아탈지 판단 필요)

## 138. 3차 Q&A 세션(2026-09-22, 지도교수 라이브) -- 회전 셋업 물리구조 확인+센서 스펙 확정+AER 설계철학 재확인

**자료**: 회의록 전문+스크린샷+PPT+데이터셋을 `Q&A/3차/`에 정리(`3차_QnA_회의록_2026-09-22.md`에 핵심요약 포함, git 미커밋 -- 대용량+민감정보 가능성).

**우리 설계에 직결되는 핵심 사실**:
- **회전 셋업 물리 구조 확인**: 센서가 자체 회전하는 게 아니라 **봉(rod) 끝에 매달려 봉이 피벗을 중심으로 원호를 그리며 도는 구조**("지구-달" 비유) -- 우리 `coord_transform_model.py`의 `world_center(θ)=(Wc+R·cosθ, Hc+R·sinθ)` 원호 모델이 임의 단순화가 아니라 **실제 물리 셋업과 구조적으로 일치**함이 확인됨. 데이터셋 2종 중 하나는 이 봉 전체가 다시 더 큰 회전판에 얹힌 2단 회전(반경 다름).
- **센서 스펙 최초 확정**: 해상도 **960×720**(1000×1000 아님), 렌즈 초점거리 **6mm**(mc2 저가 보안카메라용). §delta_sdk_discovery에서 추정했던 "~960컬럼"이 실측으로 확인.
- **AER 타임스탬프를 프레임당 1개만 찍는 설계 근거(수치 확보)**: 이벤트별 타임스탬프는 대역폭을 다 잡아먹어서 **이벤트 전송량이 4~40배 줄어듦** -- 그래서 희생. 그렇게 해도 최악 오차 **~60μs 이내**로 묶임(실측). 이벤트별 정밀 타임스탬프 자체도 허상에 가까움(어두운 배경은 픽셀 반응이 초 단위로 늦어짐, 그 지연이 타임스탬프에 섞여 들어감).
- **읽기 순서**: 좌상단→우하단 래스터 스캔(arbiter 기반 아님) -- "unfair arbiter"(우선순위 기반 읽기 시)가 읽는 시점에 따라 영상을 왜곡시키는 문제를 피하려는 의도적 설계, 지도교수 본인 논문(P4/P5, [[project_advisor_papers_analysis]])의 문제의식과 동일.
- **매핑 정확도는 러프해도 된다** 재확인 -- "이런 정보는 상대적으로 러프해도 큰 문제는 없다, 효율적으로 변환하는 방법이 주요 관심사".
- **월드메모리 저장값 자유**, **Ground Truth는 별도 안 주고 장면의 물리적 제약(막대는 지면에 수직)에서 참가자가 직접 유추**, **2D만으로 회전행렬 추정 가능성 긍정**, **렌즈왜곡 보정은 안 해도 되지만 중요하다고 생각하면 근거 제시**.
- 아날로그 트랙 참고: post-layout sim은 256×256 대신 4×4(2×2 확장)로 충분(주변 대칭 가정), 실제 어레이는 가장자리에 8행/열 더미 추가.

- 신규(git 미커밋, 대용량/민감정보): `Q&A/3차/3차_QnA_회의록_2026-09-22.md`, `Q&A/3차/rotation_setup.pptx`, `Q&A/3차/3rd_qna_DELTA_dataset.zip`, `Q&A/3차/KakaoTalk_*.png`(팀 QnA 질문 스크린샷), `Q&A/2026-09-15_*`(DELTA SDK 관련, §delta_sdk_discovery와 동일 자료)
- 다음: 없음(정보 확보 완료, 필요시 §134 로봇팔 시나리오/§136 검증에 이 사실들 반영)
